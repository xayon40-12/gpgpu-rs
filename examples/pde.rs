...
fn simple_int() -> gpgpu::Result<()> {
    ...
    let mut gpu = Handler::builder()?
        ...
        ],None,vec![("a".into(),CF64),("t".into(),CF64)]))
        .build()?;

    let args = vec![("a".to_string(),F64(1.0))];
    let mut ip = IntegratorParam { t: 0.0, increment_name: "t".into(), args };
...
fn diffusion_int() -> gpgpu::Result<()> {
    ...
    let mut gpu = Handler::builder()?
        ...
        ],None,vec![("D".into(),CF64),("ivdx".into(),CF64),("t".into(),CF64)]))
        .build()?;
...
fn diffusion_int_vect() -> gpgpu::Result<()> {
    ...
    let mut gpu = Handler::builder()?
        ...
        ],None,vec![("t".into(),CF64)]))
        .build()?;

    let mut ip = IntegratorParam { t: 0.0, increment_name: "t".into(), args: vec![] };
    let bufs = ["u","swu"];
    let m = (t/dt) as usize;
    let start = SystemTime::now();
    for i in 0..m {
