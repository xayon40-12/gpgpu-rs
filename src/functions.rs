...
pub fn fix_newton<'a>(name: &'a str, f: &'a str, extra_param_f: &'a[&'a str], e: f64, max_iter: i32) -> SFunction {
    let mut extra = extra_param_f.join(",");
    if extra.len() > 0 { extra.insert(0,','); }
    Function {
        name: &name,
        args: extra_param_f.iter().fold(vec![FCParam("x",CF64)], |mut acc,i| { acc.push(FCParam(i,CF64)); acc }),
        ret_type: Some(CF64),
        src: &format!(
            "    const double e = {};
    const double h = {};
    ...
    return x;", e, e/10.0, 10.0/e, max=max_iter, f=f, extra=extra),
...
use crate::kernels::Kernel;
    ...
    assert_eq!(gpu.get("u")?.VF64(), (0..num).map(|j| null_if_nan((f64::sqrt(j as f64 -1.0)*1000.0).floor()/1000.0) ).collect::<Vec<_>>());
