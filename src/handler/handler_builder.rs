use crate::algorithms::{self, Algorithm, Callback, SAlgorithm, SNeeded};
...
impl HandlerBuilder {
    ...
    fn add_function(
        ...
        if let Some(as_name) = as_name {
            if let Some(from) = self.existing_functions.get(&as_name) {
                panic!(
                    "Cannot add two functions with the same name \"{}\", already added by {}.",
                    as_name, from
                );
            } else {
                self.existing_functions
                    .insert(as_name.clone(), "User".into());
                self.functions.push((as_name, function));
            }
        } else if let Some(from) = self.existing_functions.get(&name) {
            if from == "User" {
                panic!(
                    "Cannot add two functions with the same name \"{}\", already added by User.",
                    name
                );
            } else {
                return self;
    ...
    fn add_kernel(
        mut self,
        kernel: impl Into<SKernel>,
        as_name: Option<String>,
        from_alg: Option<String>,
    ) -> Self {
        ...
        if let Some(as_name) = as_name {
            ...
            if from == "User" {
                panic!(
                    "Cannot add two kernels with the same name \"{}\", already added by User.",
                    name
                );
            } else {
                return self;
            ...
            self.kernels
                .insert(name.clone(), (kernel, from_alg.unwrap_or("".into()))); //TODO verify if empty string here causes problem
        }
        for n in needed {
            self = match n {
                FSNeeded::FuncName(name) => self.load_function(&name),
                FSNeeded::CreateFunc(func) => {
                    self.add_function(func, None, Some(format!("kernel \"{}\"", name)))
                }
            }
        }
    ...
    pub fn load_algorithm_named(self, name: &str, as_name: &str) -> Self {
        assert_ne!(
            name, as_name,
            "Names must be different for method \"load_algorithm_named\", given name \"{}\"",
            name
        );
        let alg = self
            .available_algorithms
    ...
    fn add_algorithm(
        ...
        if let Some(as_name) = as_name.clone() {
            ...
            if from == "User" {
                panic!(
                    "Cannot add two algorithms with the same name \"{}\", already added by User.",
                    name
                );
            } else {
                return self;
            ...
            self.algorithms
                .insert(name.clone(), (callback, from_alg.unwrap_or("".into())));
            //TODO verify if empty string here causes problem
        }
        for n in needed {
            self = match n {
                ...
                SNeeded::KernelName(n) => {
                    ...
                    let kernel = self
                        .available_kernels
                        .get(s)
                        .expect(&format!("kernel \"{}\" not found", n))
                        .clone();
                ...
                SNeeded::AlgorithmName(n) => {
                    ...
                    let alg = self
                        .available_algorithms
                        .get(s)
                        .expect(&format!("algorithm \"{}\" not found", n))
                        .clone();
    ...
    pub fn source_code(&self) -> String {
        ...
        for (name, (SKernel { src, args, .. }, ..)) in &self.kernels {
            ...
            prog.pop(); // remove last unnescessary ","
            prog += ") {\n";
            prog += "    int x = get_global_id(0); int x_size = get_global_size(0);\n";
            prog += "    int y = get_global_id(1); int y_size = get_global_size(1);\n";
            prog += "    int z = get_global_id(2); int z_size = get_global_size(2);\n";
            prog += src;
            prog += "\n}\n";
        }
    ...
    pub fn build(self) -> ocl::Result<super::Handler> {
        ...
        for (name, desc) in self.buffers {
            let existing = match &desc {
                BufferConstructor::Len(val, len) => {
                    buffers.insert(name.clone(), val.gen_buffer(&pq, *len)?)
                }
                BufferConstructor::Data(data) => {
                    buffers.insert(name.clone(), data.gen_buffer(&pq)?)
                }
            };
            if existing.is_some() {
        ...
        for (name, (SKernel { args, .. }, ..)) in self.kernels {
            ...
            for a in args {
                match a {
                    ...
                    SKernelConstructor::KCBuffer(n, b)
                    | SKernelConstructor::KCConstBuffer(n, b) => {
                        map.insert(n.to_string(), id);
                        id += 1;
