...
use gpgpu::descriptors::{
    BufferConstructor::*, ConstructorTypes::*, KernelArg::*, KernelConstructor::*, Types::*,
    VecTypes::*,
};
...
fn simple_main() -> gpgpu::Result<()> {
    ...
    for i in 0..10 {
        ...
        assert_eq!(
            gpu.get("u")?.VF64(),
            (0..num)
                .map(|j| i as f64 + num as f64 * 1000.0 + j as f64 * 100.0)
                .collect::<Vec<_>>()
        );
    }
...
fn plus() -> gpgpu::Result<()> {
    ...
    let mut gpu = Handler::builder()?
        .add_buffer(
            "a",
            Data((0..num).map(|i| i as f64).collect::<Vec<_>>().into()),
        )
    ...
    gpu.run_arg(
        "plus",
        Dim::D1(num),
        &[Buffer("a"), Buffer("b"), Buffer("dst")],
    )?;
    assert_eq!(
        gpu.get("dst")?.VF64(),
        (0..num).map(|i| i as f64 + 2.0).collect::<Vec<_>>()
    );
...
fn sum() -> gpgpu::Result<()> {
    ...
    assert_eq!(
        gpu.get_first("dstxy")?.F64(),
        v.iter().fold(0.0, |a, i| i + a),
        "dim XY"
    );
    ...
    assert_eq!(
        gpu.get("dsty")?.VF64(),
        v.chunks(x).fold(vec![0f64; x], |a, c| a
            .iter()
            .enumerate()
            .map(|(i, v)| v + c[i])
            .collect::<Vec<_>>()),
...
fn min() -> gpgpu::Result<()> {
    ...
    assert_eq!(
        ...
        v.chunks(x)
            .map(|b| b
                .iter()
                .fold(std::f64::MAX, |a, &i| if i < a { i } else { a }))
            .collect::<Vec<_>>()
    );
...
fn max() -> gpgpu::Result<()> {
    ...
    assert_eq!(
        ...
        v.chunks(x)
            .map(|b| b
                .iter()
                .fold(std::f64::MIN, |a, &i| if i > a { i } else { a }))
            .collect::<Vec<_>>()
    );
...
fn correlation() -> gpgpu::Result<()> {
    ...
    assert_eq!(
        gpu.get("dst")?.VF64(),
        (0..num)
            .map(|i| i as f64 * ((i / x) * x + x / 2) as f64)
            .collect::<Vec<_>>(),
        "dim X"
    );
    ...
    assert_eq!(
        gpu.get("dst")?.VF64(),
        (0..num)
            .map(|i| i as f64 * ((i % x) + x * (y / 2)) as f64)
            .collect::<Vec<_>>(),
        "dim Y"
    );
    gpu.run_algorithm(
...
fn fft() -> gpgpu::Result<()> {
    ...
    let mut gpu = Handler::builder()?
        .add_buffer(
            "src",
            Data(VF64_2((0..num).map(|i| [i as f64, 0.0].into()).collect())),
        )
    ...
    assert_eq!(
        gpu.get("dst")?
            ...
            .collect::<Vec<_>>(),
        (0..num)
            .map(|i| [i as f64, 0.0].into())
            .collect::<Vec<[f64; 2]>>()
            .chunks(x)
            .flat_map(|c| ft(c))
            .map(|d| cut2([d[0] / x as f64, d[1] / x as f64].into()))
            .collect::<Vec<_>>(),
        "dim X"
    );
    ...
    assert_eq!(
        ...
        {
            let tmp = (0..num)
                .map(|i| [((i % y) * x + i / y) as f64, 0.0].into())
                .collect::<Vec<[f64; 2]>>()
                .chunks(y)
                .flat_map(|c| ft(c))
                .map(|d| [d[0] / y as f64, d[1] / y as f64].into())
                .collect::<Vec<[f64; 2]>>();
            (0..num)
                .map(|i| cut2(tmp[(i * y) % (x * y) + i / x]))
                .collect::<Vec<[f64; 2]>>()
        },
        "dim Y"
    );
    ...
    let local = {
        let tmpp = (0..num)
            ...
            .chunks(x)
            .flat_map(|c| ft(c))
            .map(|d| [d[0] / x as f64, d[1] / x as f64].into())
            .collect::<Vec<[f64; 2]>>();
        let tmp = (0..num)
            .map(|i| tmpp[((i % y) * x + i / y)])
            .collect::<Vec<[f64; 2]>>()
            .chunks(y)
            .flat_map(|c| ft(c))
            .map(|d| [d[0] / y as f64, d[1] / y as f64].into())
...
fn moments() -> gpgpu::Result<()> {
    ...
    let mut gpu = Handler::builder()?
        .add_buffer(
            "src",
            Data(VF64((0..num).map(|i| (i % x) as f64).collect())),
        )
        .add_buffer("tmp", Len(F64(0.0), num))
    ...
    assert_eq!(
        ...
        pow.chunks(x)
            .map(|c| c
                .into_iter()
                .fold([0.0, 0.0, 0.0, 0.0], |[a, b, c, d], (e, f, g, h)| [
                    a + e,
                    b + f,
                    c + g,
                    d + h
    ...
    assert_eq!(
        gpu.get("dsty")?.VF64(),
        pow.chunks(x * z)
            .fold(vec![(0.0, 0.0, 0.0, 0.0); x * z], |a, c| a
                .iter()
                .enumerate()
                .map(|(i, v)| (v.0 + c[i].0, v.1 + c[i].1, v.2 + c[i].2, v.3 + c[i].3))
                .collect())
            .iter()
...
#[test]
#[should_panic(
    expected = "Cannot add two algorithms with the same name \"sum\", already added by algorithm \"moments\"."
)]
fn load_algorithm_already_created() {
    let _gpu = Handler::builder()
        .unwrap()
        .load_algorithm("moments")
...
#[test]
#[should_panic(
    expected = "Cannot add two kernels with the same name \"times\", already added by algorithm \"moments\"."
)]
...
#[test]
fn sort() -> gpgpu::Result<()> {
    let n = 64;
    let mut gpu = Handler::builder()?
        .add_buffer("src", Data(VF64((0..n).map(|x| x as f64).collect())))
        .add_buffer("tmp", Len(F64(0.0), n))
        .add_buffer("dst", Len(F64(0.0), n))
        .add_buffer(
            "srcF",
            Data(VF64_2((0..n).map(|x| [x as f64, -(x as f64)]).collect())),
        )
        .add_buffer("tmpF", Len(F64_2([0.0, 0.0]), n))
        .add_buffer("dstF", Len(F64_2([0.0, 0.0]), n))
        .add_buffer(
            "srcS",
            Data(VF64_2((0..n).map(|x| [x as f64, -(x as f64)]).collect())),
        )
        .add_buffer("tmpS", Len(F64_2([0.0, 0.0]), n))
        .add_buffer("dstS", Len(F64_2([0.0, 0.0]), n))
        .load_algorithm("sort")
        .load_algorithm("sortByFirst")
        .load_algorithm("sortBySecond")
        .build()?;

    gpu.run_algorithm("sort", Dim::D1(n), &[X], &["src", "tmp", "dst"], Nothing)?;
    let res = gpu.get("dst")?.VF64();
    assert_eq!(res, (0..n).map(|x| x as f64).collect::<Vec<_>>());
    ...
    let fres = gpu.get("dstF")?.VF64_2();
    assert_eq!(
        fres,
        (0..n).map(|x| [x as f64, -(x as f64)]).collect::<Vec<_>>()
    );
